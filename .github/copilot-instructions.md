...
- **IMPORTANT: Never commit automatically** - make code changes but ALWAYS wait for explicit "commit" instruction from the user
...
- **NEVER commit automatically** - this is critical
- Always make code changes but DO NOT run git commit unless the user explicitly says "commit"
- Test changes in the browser first
- Wait for explicit "commit" instruction from the user before running any git commands
- This ensures the user reviews all changes before they are committed to the repository
